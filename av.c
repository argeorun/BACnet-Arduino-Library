/**
 * @file av_m.c
 * @brief Arduino-compatible copy of `av.c` (preserves all function names/signatures)
 *
 * This file was created as a drop-in copy of `atmega328/av.c` for use
 * in Arduino projects. The code itself was left unchanged to preserve
 * all function names and return types exactly as requested.
 *
 * Notes for porting to Arduino:
 * - The following functions are hardware/platform dependent and must
 *   have Arduino-compatible implementations (either by providing
 *   replacements with the same signatures or by adding wrapper functions):
 *     - `adc_millivolts(uint8_t channel)`
 *     - `adc_enable(uint8_t channel)`
 *     - `nvdata_unsigned8`, `nvdata_unsigned8_set`
 *     - `nvdata_unsigned24`, `nvdata_unsigned24_set`
 *     - `RS485_Baud_Rate_From_Kilo(uint8_t kbaud)`
 *     - `stack_size()` and `stack_unused()`
 * - The include headers are preserved; ensure the Arduino project
 *   supplies compatible header files or adjusts include paths.
 * - No AVR-GCC builtins (ISR, sei, cli, PROGMEM, etc.) were found in
 *   the original `av.c`, so no direct AVR intrinsic replacements were
 *   necessary in this file. Hardware interaction is delegated to the
...
void Analog_Value_Init(void)
{
    ...
    /* Set runtime-only properties that cannot be compile-time constants */
    {
        struct object_data *obj = Object_List_Element(96); /* MCU Frequency */
        if (obj) {
            obj->present_value = mcu_frequency_float();
        }
    }
}
